...
const categories = [
  { name: "Parturit", query: "Parturi", icon: Scissors, count: "12 tekijää" },
  { name: "Kampaamot", query: "Kampaamo", icon: Sparkles, count: "18 tekijää" },
  { name: "Kynnet", query: "Kynnet", icon: Paintbrush, count: "9 tekijää" },
  { name: "Ripset", query: "Ripset", icon: Eye, count: "7 tekijää" },
  { name: "Meikkaus", query: "Meikkaus", icon: Star, count: "6 tekijää" },
  { name: "Kotitekijät", query: "hobbyist", icon: House, count: "8 tekijää", type: true },
];
  ...
  return <>
    <section className="relative overflow-hidden bg-[#f8f7f3] py-14 md:py-24">
      ...
      <div className="container-page relative grid items-center gap-12 lg:grid-cols-[1.2fr_.8fr]">
        <div>
          ...
          <p className="mt-6 max-w-2xl text-base leading-7 text-black/60 md:text-lg">Parturit, kampaamot, kynsiteknikot ja uudet kauneusalan lahjakkuudet ympäri Suomen — samassa selkeässä haussa.</p>
          <HeroSearch/>
        ...
        <div className="relative hidden min-h-[480px] lg:block" aria-hidden="true">
          ...
          <div className="absolute bottom-0 right-0 w-64 rotate-3 rounded-[28px] bg-white p-5 shadow-2xl"><div className="h-28 rounded-2xl bg-gradient-to-br from-[#21202a] to-[#8079a2]"/><div className="mt-4 flex items-center justify-between"><div><b className="text-sm">Nails by Nea</b><p className="text-xs text-black/45">Jyväskylä</p></div><span className="flex items-center gap-1 text-xs font-bold"><Star className="size-3 fill-[#6d4aff] text-[#6d4aff]"/>4.7</span></div></div>
          <div className="absolute right-4 top-16 rounded-2xl bg-[#17151d] px-4 py-3 text-white shadow-xl"><p className="text-[10px] text-white/50">Vapaita aikoja</p><p className="text-sm font-bold">Tänään klo 16.30</p></div>
        </div>
      </div>
    </section>
    ...
    <section className="container-page py-16 md:py-20">
      ...
      <div className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        {categories.map(({name, query, icon: Icon, count, type}) => <Link key={name} href={`/search?${type ? "type" : "category"}=${encodeURIComponent(query)}`} className="group rounded-[22px] border border-black/8 p-5 transition hover:-translate-y-1 hover:border-[#6d4aff]/30 hover:bg-[#f5f2ff]">
          <div className="grid size-11 place-items-center rounded-2xl bg-[#f0edff] text-[#6d4aff] transition group-hover:bg-[#6d4aff] group-hover:text-white"><Icon className="size-5"/></div><b className="mt-7 block text-sm">{name}</b><span className="mt-1 block text-xs text-black/40">{count}</span>
        </Link>)}
      </div>
    </section>
    ...
    <section className="bg-[#17151d] py-20 text-white">
      <div className="container-page grid gap-10 lg:grid-cols-[.8fr_1.2fr]">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{cities.map((city, index) => <Link key={city} href={`/search?city=${city}`} className={`flex items-center justify-between rounded-2xl border border-white/10 px-4 py-4 text-sm font-bold transition hover:bg-white hover:text-black ${index === 0 ? "border-transparent bg-[#6d4aff] sm:col-span-2" : ""}`}>{city}<MapPin className="size-4 opacity-50"/></Link>)}</div>
      </div>
    </section>
